...
def is_valid_digitslate_address(address, network='mainnet'):
    ...
    digitslate_version = 112 if network == 'testnet' else 80
    ...
    try:
        ...
        # rescue from exception, not a valid DigitSlate address
        return False

    if (address_version != digitslate_version):
        return False
...
# shims 'til we can fix the digitslated side
def SHIM_serialise_for_digitslated(sentinel_hex):
    from models import DIGITSLATED_GOVOBJ_TYPES
    # unpack
    obj = deserialise(sentinel_hex)

    # shim for digitslated
    govtype = obj[0]

    # add 'type' attribute
    obj[1]['type'] = DIGITSLATED_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in digitslated
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # digitslated expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    digitslated_hex = serialise(obj)
    return digitslated_hex


# shims 'til we can fix the digitslated side
def SHIM_deserialise_from_digitslated(digitslated_hex):
    from models import DIGITSLATED_GOVOBJ_TYPES

    # unpack
    obj = deserialise(digitslated_hex)

    # shim from digitslated
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('digitslate.conf')
