...
class GovernanceObject(BaseModel):
    ...
    # sync digitslated gobject list with our local relational DB backend
    @classmethod
    def sync(self, digitslated):
        golist = digitslated.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_digitslated(digitslated, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_digitslated(self, digitslated, rec):
        import decimal
        ...
        # shim/digitslate conversion
        object_hex = digitslatelib.SHIM_deserialise_from_digitslated(object_hex)
        objects = digitslatelib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from digitslated...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from digitslated, with every run
        ...
        # get/create, then sync payment amounts, etc. from digitslated - DigitSlated is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid(digitslated) is False:
                govobj.vote_delete(digitslated)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from digitslated! %s" % e)
            govobj.vote_delete(digitslated)
            return (govobj, None)
    ...
    def vote_delete(self, digitslated):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(digitslated, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, digitslated, signal, outcome):
        import digitslatelib
        ...
        output = digitslated.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = digitslatelib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(digitslated, signal)

    def sync_network_vote(self, digitslated, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = digitslated.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = DIGITSLATED_GOVOBJ_TYPES['proposal']

    class Meta:
        db_table = 'proposals'

    def is_valid(self, digitslated):
        import digitslatelib
        ...
        try:
            ...
            # payment address is valid base58 digitslate addr, non-multisig
            if not digitslatelib.is_valid_digitslate_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid DigitSlate address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (digitslatelib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/DigitSlateDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import digitslatelib
        obj_data = digitslatelib.SHIM_serialise_for_digitslated(self.serialise())
    ...
    def prepare(self, digitslated):
        try:
            object_hash = digitslated.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = DIGITSLATED_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self, digitslated):
        ...
        for addr in addresses:
            if not digitslatelib.is_valid_digitslate_address(addr, config.network):
    ...
    def hash(self):
        import digitslatelib
        return digitslatelib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = DIGITSLATED_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, digitslated):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - digitslated.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, digitslated):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - digitslated.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, digitslated):
        now = int(time.time())
        return (self.created_at < (now - digitslated.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, digitslated):
        if self.is_expired(digitslated):
            return False

        return True

    def is_deletable(self, digitslated):
        if self.is_expired(digitslated):
            return True
