...
def has_digitslate_conf():
    import config
    import io

    valid_digitslate_conf = False

    # ensure digitslate_conf exists & readable
    #
    # if not, print a message stating that DigitSlate Core must be installed and
    # configured, including JSONRPC access in digitslate.conf
    try:
        f = io.open(config.digitslate_conf)
        valid_digitslate_conf = True
    except IOError as e:
        print(e)

    return valid_digitslate_conf
...
def main():
    ...
    if not has_digitslate_conf():
        print("DigitSlate must be installed and configured, including JSONRPC access in digitslate.conf")
        sys.exit(1)
