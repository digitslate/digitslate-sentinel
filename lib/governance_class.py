...
class GovernanceClass(object):
    ...
    def vote(self, digitslated, signal, outcome):
        return self.go.vote(digitslated, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, digitslated):
        if self.is_valid(digitslated):
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(digitslated, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(digitslated, models.VoteSignals.valid, models.VoteOutcomes.no)
    ...
    def list(self):
        ...
        # return a dict similar to digitslated "gobject list" output
    ...
    def digitslated_serialise(self):
        import digitslatelib
        return digitslatelib.SHIM_serialise_for_digitslated(self.serialise())
