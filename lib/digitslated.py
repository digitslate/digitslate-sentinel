...
class DigitSlateDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some digitslated methods
    ...
    @classmethod
    def from_digitslate_conf(self, digitslate_dot_conf):
        from digitslate_config import DigitSlateConfig
        config_text = DigitSlateConfig.slurp_config_file(digitslate_dot_conf)
        creds = DigitSlateConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = digitslatelib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = digitslatelib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = digitslatelib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_digitslated_proto_version_with_sentinel_ping)
